...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    println!(
        "Starting {} node with WebSocket port {} and P2P port {}",
        node_name, ws_port, p2p_port
    );
    ...
    // Start the P2P node
    let p2p_sender = p2p::start_p2p_node(
        node_name.clone(),
    ...
    while let Ok((stream, _)) = listener.accept().await {
        ...
        tokio::spawn(async move {
            handle_connection(stream, game_state_clone, node_name_clone).await;
        });
    }
